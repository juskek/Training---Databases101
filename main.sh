...
while true; do
    ...
    options=("$connect" "$schema" "$get_pokemon_with_species_name" "$get_species_with_type" "$create"  "$init_pokemon_species" "$init_pokemon_table" "$init_pokemon_type_table" "$delete_db" "$exit")
